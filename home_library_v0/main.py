...
# models.py에서 정의한 Book, ReadingStatus, Review 클래스들을 실제 PostgreSQL 테이블로 생성하는 역학
...
# ------------------------------------------------------------------------------------------------------
# POST /books/scan --> 사진을 업로드해서 책 한 권을 등록하는 API  --> Version2 (진짜 이미지인지 검증)
# ------------------------------------------------------------------------------------------------------ 
...
def scan_book(image: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    # UnidentifiedImageError --> PIL이 아예 이미지 형식으로조차 인시갛지 못했을 때 발생하는 에러
    # OSError  --> 파일이 중간에 잘렸거나, 손상된 경우 등 좀 더 넓은 범위의 파일 관련 에러
        
    except (UnidentifiedImageError, OSError):
        # HTTP 상태코드가 415(Unsupported Media Type, 지원하지 않는 파일 형식) 함수 실행 중단 시킨다.
        # raise를 만나는 순간 저장, DB등록은 실행되지 않는다.
        raise HTTPException(415, '올바른 이미지 파일이 아닙니다.')

    # uploads 폴더 밑에 "원래 업로드 된 파일명"
    ...
    return book     # book 객체를 자동으로 JSON으로 변환해서 Streamlit에 돌려준다.
