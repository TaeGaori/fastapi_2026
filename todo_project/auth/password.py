...
pwdlib라이브러리 사용 -> 장점 : 내부적으로 salt(무작위 값)를 자동을 섞어서 해싱하기 때문에
                        같은 비밀번호를 넣어도 매번 다른 해시값이 생서된다.
                        공격자가(해커)가 미리 계산해둔 해시 목록으로 대조하는 공격을 막을 수 있다.
                        FastAPI에서 권장하는 최신 보안 라이브러리
========================================================================
'''
from pwdlib import PasswordHash

# PasswordHash.recommended() : pwdlib라이브러리가 현재 가장 안전하다고 권장하는 해시 알고리즘을 자동으로 선택
#   (기본적으로 Argon2id-bcrypt보다 CPU 기반 무차별 대입 공격에 더 강한 최신 알고리즘)
