# ================================================
# NCS-bigdata_porcessing_system/batch_processor.py
#   - "배치 처리(Batch Processing)" 단계 예제 실습
#   - 저장시스템에 이미 적재되어 있는 원본 테이블(subway_raw, bus_stop) 전체를 한 번에 읽어서, 집계 결과 테이블을 새로 만든다.
# 
# 
#   --> 배치 처리의 특징 : "전체 데이트를 대상으로, 정해진 시점에 한 번에" 처리
#   이번 예제에서는 Python 코드가 데이터를 직접 계싼하지 않고,
...
def run_batch_processing() -> None:
    ...
    작은 단위로 쪼개고, 함수가 순서대로 호출하는 구조는 pipline.py에서
    batch / realitime / event 단계를 조립할 때 재사용하기 쉽다.
    """
...
# (다른 파일에서 import batch_processor로 불러올 때는 실행되지 않는다.
#   모듈 재사용을 위한 관용구)
