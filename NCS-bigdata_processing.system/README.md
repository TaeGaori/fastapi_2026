...
## 대구 교통 데이터 처리시스템 예제
저장시스템 실습에서 만든 PostreSQL 테이블을 이용하여 배치 처리, 실시간 처리, 이벤트 처리 모듈을 실행하는 예제
...
```bash
set SUBWAY_DB_URL=postgresql://postres:1234@localhost:5432/subwaydb
set BUS_DB_URL=postgresql://postgres:1234@localhost:5432/busaopdb
```
...
SELECT * FROM traffic_subway_event_alerts ORDER BY detected_at DECS LIMIT 20;
```

```sql
SELECT * FROM traffic_bus_area_summary ORDER BY stop_count DESC;
SELECT * FROM traffic_bus_event_alerts ORDER BY detected_at DECS LIMIT 20;
```
