# ================================================
# NCS-bigdata_porcessing_system/event_processor.py
#   -"이벤트처리(Event Processing / CEP)" 예제
#       배치 처리 결과와 원본 데이터를 대상으로 "이상 상황(조건을 만족하는 시건)"을
#       탐지하여 별도의 알림(alert) 테이블에 기록한다.
# 
#   - 이벤트 처리는 조건(규칙)을 만족하는 사건을 찾아내느 것이 핵심이다.
...
#       테이블명, 컬러명, ASC/ DESC 같은 "SQL 구조"는 보통 값 파라미터로 바인딩할 수 없다. 
#       --> 허용 목록(화이트리스트)으로 검증
# 
#   3) 멱등성(idempotency)
#       같은 초기화 코드를 여러번 실행해도 결과가 망가지지 않는다. 
# ================================================
import argparse # 파이썬 그로그램을 실행할 때 터미널에서 존달한 옵션을 읽는 표준 라이브러리
from datetime import datetime

from sqlalchemy import text

from config import BUS_LOCATION_GROUPS, DAEGU_LAT_MAX, DAEGU_LAT_MIN, DAEGU_LON_MIN, DAEGU_LON_MAX
...
def init_subway_alert_table() -> None:
    ...
    컬럼 설명:
        ...
        threshold_value : 이벤트로 판정된 기분값(임계값)
...
def detect_subway_congestion(threshold: int) -> None:
    ...
    execute_sql(
        subway_engine,
        """
        DELETE FROM traffic_subway_event_alerts WHERE event_type = 'SUBWAY_CONGESTION';
        """
    )
    ...
    print(f'[event] 지하철 혼잡 이벤트 남지 완료 threshold={threshold:,}')


def check_subway_summary_ready() ->None:
    """
    이벤트 처리는 배치 처리 결과(traffic_subway_horuly_summary)를 입력으로 사용하므로
    이 테이블이 먼저 만들어져야 한다.
    """
    try:
        ...
        raise RuntimeError(
            "traffic_subway_horly_summary 테이블이 없습니다."
...
def detect_bus_coordinate_events() -> None:
    """
    버스정류소 원본(bus_stop) 데이터에서 두 종류 이상 이벤트를 탐지

    1) BUS_COORDINATE_OUT_OF_RANGE
        위도/경도가 NULL이거나, 대구 범위를 벗어난 정류소를 찾는다.

    2) BUS_LOCATION_GROUP_INVALID
        "위치구분" 값이 NULL이거나, BUS_LOCATION_GROUPS에 없는 값이면 이상데이터로 간주
...
def parse_args() -> argparse.Namespace:
    ...
    -> 임계값을 50000명으로 낮춰서 더 많은 혼잡 이벤트를 탑지하도록 실행

    # ArgumnetParser 객체 생성
    ArgumnetParser(description='설명글')
