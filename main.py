...
@app.get(
    ...
    status_code=status.HTTP_200_OK      # 성공 시 200을 반환
)
def get_todos_handler():
    session = SesstionFactory()
    try:    # stmt : SQL문을 의미하는 statement의 약자
        stmt = select(Todo) # 데이터를 조회하는 쿼리 삭제(아직 데이터베이스에는 접근 안했다)
        # session.execute(stmt) : 쿼리 객체를 실제 데이터베이스에 전달해서 실행
        # .scalar().all() : 실행 결과에서 테이블의 각 행에 대응되는 ORM 객체를 추출해서 리스트로 
        todos = session.execute(stmt).scalar().all()    # 전체 결과 리스트로 반환
        return todos
    finally: # 예외가 발생하든 안한든 무조건 실행
        session.close()
