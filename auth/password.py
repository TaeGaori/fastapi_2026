...
# - pwdlib 라이브러리를 사용
#   - 내부적으로 무작위값을 자동으로 섞어서 해싱하기 대문에 같은 비밀번호라도 매번 다른 해시값이 생성이 된다.

# 해싱 (hashing)
# - 입력값을 일방향(one-way) 알고리즘을 통해 완전히 다른 문자열로 변환하는 과정
# - 이렇게 생성된 결과값을 해시(hash)라고 한다.
# - 변환된 값을 다시 원래 값으로 되될릴 수 없다.
# - 해시값만 가지고 원래의 입력값으로 역으로 계산하는 것은 불가능하다.
# - 인증은 원본 비밀번호가 아닌 해시값을 기준으로 이루어진다.
# - FastAPI 공식 문서에는 argon2 비밀번호 해싱 알고리즘을 권장
#       --> GPU 병렬 연산이나 메모리 기반 공격 등 최신 공격 기법을 고려해 설게된 현대적인 알고리즘
