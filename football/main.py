...
# football/main.py
# 
# 5장 - AIP 문서화 (처음 보는 다른 개발자가 / docs(Swagger UI)만 보고도 어떻게 사용하는지
...
#       tags : Swgger UI에서 엔드포인트들을 그룹으로 묶여 보여주는 기준이다.
# ======================
...
## 멤버십(Membership)
SWC 판타지 풋볼 리그 전체와 각 리그에 속한 ㅣㅁ에 대한 정보를 제공합니다.
'''
...
@app.get('/',
        ...
        #  response_description : 이 API를 호출하면 어떤 응답이 오는가에 설명 문구
...
@app.get('/v0/players/', response_model=list[schemas.Player],
        ...
        이름은 고유하지 않습니다. skip과 limil를 사용해 API 페이징을 수행합니다.
        선수 수를 셀 때는 Player ID값을 사용하지 않습니다
        ID는 순차적으로 진행되지 않습니다.''',
...
def read_players(skip : int = Query(0, description='API 호출 시작 부분에서 건너뛸 항목 수입니다.'),
                ...
                last_name:str = Query(None, description='반환할 순서의 성입니다.'), 
...
@app.get('/v0/players/{player_id}', response_model=schemas.Player,
        ...
        description='다른 API호출(v0_get_players 등)에서 얻은 SWC 선수 ID를 이용해 해당 선수를 조회할 수 있습니다.',
...
def read_performances(skip: int = Query(0, description='API 호출 시작 부분에서 건너뛸 항목 수입니다.'),
                    ...
                    db: Session = Depends(get_db)):
    """선수 성적 목록 조회"""
    performances = crud.get_preformances(db, skip=skip, limit=limit,
                                         min_last_changed_date=minimum_last_changed_date)
    return performances
...
@app.get('/v0/leagues/', response_model=list[schemas.League],
        ...
        description='''이 엔드포인트를 사용해 SWC판타지 풋볼 리그 목록을 조회힙니다.
...
@app.get('/v0/teams/', response_model=list[schemas.Team],
        summary='요청 매개변수에 해당하는 SWC판타지 풋볼 팀 정보를 가져옵니다.',
        description='''이 엔드포인트를 사용해 SWC 판타지 풋볼 팀 목록을 조회합니다.
        skip과 limit를 사용해 페이징을 수행합니다. 팀 이름은 공유하지 않습니다.
        다른 쿼리에서 얻은 Team ID를 이 쿼리의 Team ID와 매칭할 수 있습니다.
        Team ID는 내부 ID 이므로 개수 계산이나 로직에 사용하지 않습니다.
        순차적이지 않습니다.''',
...
def read_teams(skip: int = Query(0, description='API 호출 시작 부분에서 건너뛸 항목 수입니다.'),
               limit: int = Query(100, description='건너뛴 레코드 이후 반환할 레코드 수입니다.'),
               minimum_last_changed_date: date = Query(None, description='변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다.'),
               team_name: str =  Query(None, description='반환할 팀 이름입니다. SWC전체서 고유하지 않으나, 리그 내부에서는 고유합니다.'),
...
@app.get('/v0/counts/', response_model=schemas.Counts,
        ...
        다른 조회 엔드포인트의 skip과 limit 매개변수와 함께 사용합니다. 개수를 확인하려면 다른 API대신 이 엔드포인트를 사용합니다.''',
