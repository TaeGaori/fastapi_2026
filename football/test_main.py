...
#                  올바른 url / 상태코드 / 응담구조로 동작하는지 검증
#                  이 파일이 통과해야 'API로서 완성됐다'라고 결정
#   
# TestClient
#   uvicorn 서버를 실제로 띄우지 않고도 FastAPI 앱을 호출할 수 있게 해준다.
#   main.py의 app 객체를 파있너 함수 호출 수준에서 직접 실행해주기 때문에
#   네트워크 소켓을 열지 않고도 훨씬 빠르게 같은 결과를 확인할 수 있다.

# 왜 이 도구들을 쓰는가??
# 1) Swagger UI에서 하나하나 클릭하고ㅡ 응답 결과를 눈으로 읽고, 숫자를 세는 행위는
#       느리고, 지치고, 실수가 생길 수가 있다. 이 확인 과정을 '코드로 자동화'했다.
# 2) 회귀(regression)방지
#       오늘 만든 기능이 내일 다른 코드 수정 떄문에 망가지는 것을 자동으로 잡아내기 위해
#       pytest 한 줄 치면 수십 개 테스트가 몇 초만에 실행된다. --> 증거로 확인
# 3) TestClient를 사용하는 이유는 "서버를 안 켜도 되니까"
#       진짜 서버(uvicorn)를 키려면 포트를 열고, 네트워크 소켓을 열고, 실제로 통신을 해야한다. --> 느리고, 포트 충돌 같은 부수적인 문제도 생긴다.
#       TestClient는 실제 네트워크 통신 없이 main.py의 app 객체를 파이썬 함수 호출 수준에서 그대로 실행 --> 서버를 따로 켜둘 필요가 없다 --> pytest 한 줄로 API테스트가 가능하다.
# ======================
...
def test_read_main():
    ...
    # .json()를 호출하면 응답 본문 문자열을 파이썬 딕셔너리로 피싱해준다.
...
def test_read_players():
    ...
    limit = 10000처럼 실제 선수 수(1018명)보다 훨씬 큰 값을 줘서 페이지네이션 떄문에
    일부만 잘리지 않고ㅡ 전체가 다 오는지 확인한다
    """
...
def test_read_leagues_with_id():
    ...
    # response.json()['teams]
    # schemas.League.teams: Lsit[TeamBase] 필드와 같다.
...
def test_read_teams():
    """팀 목록 조회 엔드포인트의 전체 팀 데이트를 반환하는지 확인"""
