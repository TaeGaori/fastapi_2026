...
# 
# select() 문장을 먼저 "설계"gkrh, db.scalars(stmt)로 "실행"하는 조회함수
...
def get_players(db: Session, skip: int = 0, limit: int = 100, 
    ...
    # .scalars() --> 각 행에서 첫번째 컬럼(여기서는 Player객체자체)aks Rjsotj
    #                   리스트처럼 다루게 해주는 결과 객체 반환
    # .all() --> 그 결과를 실제 파이썬 리스트로 변환해서 반환
    return db.scalars(stmt).all()    # SQL 실행

def get_preformances(db: Session, skip: int = 0, limit: int = 100,
                     min_last_changed_date: date | None = None):
...
def get_teams(db: Session, skip: int = 0, limit: int = 100,
    ...
    --------------------------------------------------------
    team.team_players --> 이 팀에 연결된 TeamPlayere(가입 기록) 목록 --> 그 TeamPlayer
        각각의 . player는 다시 "그 기록에 연결된 Player 객체"
        즉 Team -> TeamPlayer -> Player 로 두 단계를 거쳐야 선수 정보까지 닿는다.

    selectionload(models.Team.team_players)
        Team을 다 가져온 뒤, " team_players를 위한 추가 SELECT 1번" 더 날려서 한번에 채운다.
        (joinedload처럼 JOIN해서 행이 늘어나는 방식이 아니라, 별도의 IN조회를 한 번 더 하는 방식)

    .joinedload(models.TeamPlayer.player)
        그 다음 단계(TeamPlayer -> Player)는 seletinload가 만든 두 번째 쿼리 안에서
        JOin으로 같이 가져온다.

    --> 1 단계 selectinload(추가 쿼리 방식), 2단계는 joinedload(같은 쿼리 안에서 JOIN)를
...
def _count(db: Session, model:type) -> int:
    """
    특정 모델(테이블)의 전체 행 개수를 샌다.

    함수 앞에 밑줄(_count) --> 이 파일 안에서만 쓰는 내부 도우미 함수라는 뜻의 관례표시
