...
# 클래스 안에서 타입을 적을 때, 그 클래스가 파일 아래쪽에 정의되어 있어도 그냥 이름만 쓰는 것이 가능
...
# ---------------------------------------------
# Mapped[T]
#     - 이 속성을 파있너 코드에서 어떤 타입(T)로 다룰지 표시
# mapped_column(..)
#     - 실제 DB 컬럼의 자료형 / 제약조건(기본기, 인덱스 등) 설정
# relationship(...)
#     - DB 컬럼이 이나라, 연결된 다른 ORM 객체로 이동하기 위한 "파이썬 전용"속성
#     - DB에는 저장되지 않는다.
# ---------------------------------------------
from sqlalchemy.orm import Mapped, mapped_column, relationship

from database import Base # SQLAlchemy가 관리하는 
...
class Player(Base):
    ...
    #       Team <-> Teamplayer를 각각 1:N 관계로 연결한 뒤,
    #       Teamplaer를 "연결 기록이 저장된 중간 다리"로 활용
    ...
    # ---------------------------------------------
    # association_proxy
    #     team_players는 TeamPlayer(연결기록) 객체의 리스트라서 "이 선수가 속한 팀 이름들만 보고싶다"
    #     --> player.team.players[0].team.team_name ??? 너무 길다 ㅜㅜ
    
    # association_proxy('team_players', 'team')
    #     예전처럼 player.teams 라고만 써도 Team 객체 리스트를 바로 꺼내볼 수 있게 해주는"편의통로"
...
class TeamPlayer(Base):
   ...
   (team_id + player_id) --> 복합 기본키(compositeprimary key)
