...
#   따라서 테스트를 실행하기 전에 이 파일을 한 번 실행해서 CSV데이터를
...
def parse_date(value: str) -> date:
    ...
    CSV에서 읽은 날짜 문자열을 Python date 객체로 변환한다.
        ...
        CSV파일 안에 날짜는 "2024-04-18" 같은 문자열이다.
        models.py의 last_changed_date 컬럼은 SQLAlchemy Date타입이다.
...
def seed() -> None:
    ...
    with Session(engine) as session:
        ...
        # player와 league행이 같은 트랜잭션 안에서 실제 테이블에 먼저 보이게 된다.
        # 뒤에서 team,team_player처럼 외래키를 가진 테이블을 넣을 때
        # 참조할 부모행이 아직 없다는 어류를 피하고, 실행 순서 파악도 쉽다.
        session.flush()
