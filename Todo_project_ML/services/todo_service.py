'''
========================================================================
sevices/todo_service.py

Todo 관련 "업무 규칙"을 담당하는 계층
DB 쿼리 자체는 직접하지 않고, TodoRepository에게 다 위임을 한다.
...
class TodoService:
    ...
    def get_todos(self, user_id: int) -> list[Todo]:
        ...
        # Router가 Repository를 직접 알 필요가 없다면 구조자체를 보여준다.
    ...
    def create_todo(self, body: TodoCreateRequest, user_id: int) -> Todo:
        """
        ---MLOps 확장 지점 ---
        category_service가 주입되어 있을 대만(=모델이 로드되어 있을 때만)
        제목을 넣어 카테고리를 예측한다.
        """
