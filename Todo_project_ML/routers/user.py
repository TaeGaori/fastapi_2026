...
========================================================================
'''
# BackgroundTasks : 응답을 먼저 클라이언트에게 보내고, 그 이후에도 처리해도 되는 작업을 뒤에서 실행
...
def send_welcome_email(email: str):
    ...
    시ㅣㄹ제 메일 발송 대신 5초 걸리는 작업으로 대신 <- BackgroundTasks로 등록하면
...
def login_user_handler(
    ...
    return service.login(body)  # 이메일/비밀번호번호 검증부터 토큰 발급, refresh_toke, DB저장까지 처리 후 리턴
