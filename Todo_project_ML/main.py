...
- 서버가 켜질 때/ 꺼질 때 딱 한 번씩 해야 하는 일(DB 테이블 생성, ML 모델 로드)를 처리한다
========================================================================
'''
...
# 모델 경로
# uvicorn을 어느 폴더에서 실행하느냐에 따라 상대경로 기준이 달라져 파일을 못찾는 경우가 발생한다
...
# routers/todo.py, routers/user.py에서 만든 라우터를 app하나에 "합체" 시키는 부분
