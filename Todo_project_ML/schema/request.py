...
--> 타입이 안 맞거나 필수값이 없으면 자동으로 422에러 응답.
========================================================================
'''

import re   # 정규 표현식 --> 문자열이 특정 패턴을 포함하는지 검사
from pydantic import BaseModel, EmailStr, Field, field_validator

# --- 할 일 생성 요청 모델 -------------------------------------------
class TodoCreateRequest(BaseModel):
    title: str      # 할 일의 제목, 문자열, 필수값
    is_done: bool = False   # 할 일 완료 여부 --> 기본값 False


# --- 할 일 수정 요청 모델 ---
# 두 필디 모두 Optinal인 이유 --> PATCH는 "부분 수정 허용"(부분 수정이 원칙)
...
class UserSignUpRequest(BaseModel):
    ...
    def validate_password(cls, value):
        if not re.search(r"[A-Z]", value):  # cls : 클래스 자기 자신, value는 매개변수(입력값)
