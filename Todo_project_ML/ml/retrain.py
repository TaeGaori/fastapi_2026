...
ml/retrain.py
-------------------------------------
- 사용자가 실제로 수정한 카테고리(final_category)를 새 학습 데이터로 삼아 모델로 재학습한다.
- 이 스크립트도 FastAPI 서버와 분리된 별도 프로세스로 실행한다.
    (수동 실행 -> 이후 필요하면 corn/스케줄러로 자동화)
- 시간이 지날수록 실제로 만든 Todo데이터가 쌓이면서 모델이 점점 더 우리 말투.팬터에 맞게 똑똑해지는 과정을 볼 수가 있다.
'''
import json
...
def load_original_data() -> pd.DataFrame:
    """최초 학습에 사용한 샘플 데이터. 재학습대도 꼐속 기반 데이터로 포함시킨다"""
...
def load_user_corrected_data() -> pd.DataFrame:
    ...
    FastAPI 요청 흐름 밖에서 실행되므로 SessionFactory()를 직접 호출해서 세션을 만들고, 끝나면 직접 close()한다.
    """
...
def main():
    ...
    combined_df = combined_df.drop_duplicates(subest=['title'], keep='last')
    ...
    pipeline.fit(X_train, y_trian)  # 학습
    ...
    metadata = {
        ...
        "n_user_corrected" : len(corrected_df), # 재학습 메타데이터에만이 있는 필드
        "accuracy" : round(accuracy, 4),
        "categories" : sorted(combined_df["category"].unique().tolist()),
        "source" : "original + user_corrected",
    }
    metadata_path = ARTIFACTS_DIR / f'model_v{version}_metadata.josn'
    ...
    print("[INFO] FastAPI 서버를 재시작해야 새 모델이 반영됩니다.")
