...
- FastAPI 앱(main.py) 코드와 물리적으로 완전히 분리되어 있다.
    ...
    - 학습은 몇 초~ 몇 분씩 걸릴 수 있는 무거운 작업이라, 만약 API 요청 처리 흐름 안에 학습 코드를 넣으면 그동안 서버 전체가 멈춰버린다.
'''
import json
...
# __file__.resolve().parent : 이 스크립트가 어느 폴더에 있든 항상 스크립트 자산의 위치를 기준으로 경로를 계산한다. (실행 위치에 흔들리지 않는다.)
...
def build_pipeline() -> Pipeline:
    """
    TfidVectorizer(텍스트를 -> 숫자 벡터로 변환)
    로지스틱 회귀를 하나의 Pipline 객체로 묶는다.
    --> 왜 묶어??
        서빙(FastAPI) 쪽에서 TfidVectorizer, 예측도 해야하고 ... 단계를 직접 호출해야 한다.
        두 객체를 각각 파일로 저징/로드해야 한다.
        파이프라인으로 묶으면 한 줄로 벡터화+분류가 한 번에 처리되고, 저장/로드도 파일 하나로 끝난다.

    TfidVectorizer(ngram_range(1,2)) : 단어 하나(unigram)뿐 아니라 연속된 두 단어 (bigram)까지 특징으로 사용한다.
                                        예) "회의 자료" --> "회의", "자료", "회의 자료"
...
def get_next_version(artifacts_dir: Path) -> int:
    """
    artifacts 폴더 안의 model_v1.pk1, model_v2.pkl ... 파일 명을 스캔해서 다음에 저장할 버전 번호를 계산
...
def main():
    ...
    print(df['category'].value_counts())    # 카테고리별 데이터 개수 확인(한쪽으로 쏠려있지 않은지 체크)

    # 입력(피쳐, 독립변수) --> df['title] --> X
    # 결과(타겟, 종속변수) --> df['category'] --> y
    # 전체 데이터를 "학습용", "평가용"으로 나눈다.
    # stratify=df['category'] --> 나눌 대 카테고리 비율이 원본과 비슷하게 유지되도록 강제한다.
    ...
    # joblib.dump() : 학습된 파이썬 객체(pipline)를 파일 그대로 저장
    #                 pickle과 비슷하지만, numpy배열이 많은 sklearn객체 저장에 더 최적화 되어 있다.
    ...
    # 버전이 올라갈때마다 main.py코드를 수정할 필요없이, 이 파일 하나만 최신모델로 덮어쓰기하면
    # 서버가 자동으로 최신버전을 로드하게 된다.
...
if __name__ =='__main__':
    # 이 파일을 직접 실행했을 때만 (다른 파일이 import할 때는 실행 안 되게)
    # main()이 호출되도록 하는 파이썬의 표군 관용구
    main()