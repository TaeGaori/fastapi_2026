...
#   st.session_state 재실행되어도 값이 유지되는 유일한 저장공간이다.
#   여기에 토큰/로그인 여부를 저장한다.
# -----------------------------------------------------------------------------------------
...
for todo in todos:
    ...
    # predicted가 None인 경우(=서버에 모델이 로드 안 되어 있떤 시점에 생성된 Todo)는
    # 이 영역 자체를 표시하지 않는다.
    if predicted:
        ...
        with cat_col2:
            # 확정값이 있으면 있는 값을 사용하고, 없으면 예측값을 selectbox의 촉기 선택값으로 사용
            ...
            # --> Todo.final_category를 채우는 지점 --> 나중에 ml/retain.py의 새 학습 데이터가 된다.
