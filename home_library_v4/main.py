...
#   - static 폴더 (css)
# 
# 조수를 직접 문자열로 '/static/style.css'라고 하드코딩하지 않는 이유
...
def lookup_book(isbn: str, db: Session = Depends(get_db)):
    ...
    return result.book # result.statu == 'ok' 성공
...
def register_book(isbn: str=Form(...), image: UploadFile=File(...), db: Session=Depends(get_db)):
    ...
    isbn, 책 표지를 DB에 등록
    """
    # iamge.file.read() --> 업로드 파일 내부의 실제 파일 객체를 열어서 내용을 전부 읽어 bytes(0과 1의 나열)로 가져온다
    raw = image.file.read()
    ...
    if result.status == 'invalid_image':
        # 상태 코드 415 --> Ubsupportes Media Type (파일 형식이 이미지가 아니다.)
...
def ui_lookup_form(request: Request):
    ...
    #       내부적으로 어던 요청이 어떤 서버/포트로 왔는지 등을 알아야 url_for() 같은 기능이 동작한다.
...
def ui_lookup_submit(request: Request, isbn: str=Form(...), db: Session=Depends(get_db)):
    ...
    return templates.TemplateResponse(
        request=request,
        name='lookup.html',
        # context로 넘긴 값들을 lookup.html 안에서 {{ result}}, {{ isbn_input }} 접근 가능
