...
from todo.models import SubwayRaw
...
#   내부에서 주로 사용되는 용도니, 외부에서는 직접 가져다 사용하지 말라!
def to_bool(value):
...
def prepare_chunk(chunk:pd.DataFrame) -> list[dict]:
    """
    pandas로 읽어온 CSV 한 덩어리(chunk)를, DB에 바로 넣을 수 있는 딕셔너리 리스트 형태로 가공하는 함수
    """
    ...
    chunk = chunk.dropna(subset=['역번호', '날짜', '시간대컬럼', '승하차'])     #dropna --> 결측치 제거

    # 필요한 컬럼만 순서대로 골라, DB삽입에 사용할 수 있는 딕셔너리 리스트로 변환
...
def load_from_csv(path: str=INPUT_PATH, chunksize: int=CHUNK_SIZE) -> dict:
    ...
    for i, chunk in enumerate(pd.read_csv(INPUT_PATH, encoding='utf-8-sig', chunksize=CHUNK_SIZE)):
        try:
            # 이번 배치(데이터를 한번에 처리하지 않고 일정한 묶음 단위로 처리)를 DB 삽입용 딕셔너리 리스트로 가공
            records = prepare_chunk(chunk)  # 함수 호출
            ...
            with engine.begin() as conn:
                ...
                # UNIQUE 제약에 위반되는 행은 에러를 내지 않고 그낭 무시(skip)
            ...
            # 예상치 못한 에러(예: 자료형 불일치)가 발생한 경우 이번 배치만 실패한 기록, 다음 넘어간다
...
# 이 파일을 직접 실행했을 떄만 (python loader.py) 아래 코드가 동작한다.
