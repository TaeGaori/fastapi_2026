...
# 모든 모델 클래스가 상속받은 선언적 베이스 클래스를 생성
# 이 Base를 상속받으면 클래스가 = 테이블, 클래스 속성 = 컬럼으로 자동 매핑
...
class SubwayRaw(Base):
    ...
    시간대컬럼 = Column(String(20), nullable=False) # 원본 CSV의 시간대 컬러명('05시-06시')
    ...
    # 복헙 UNIQUE 제약 조건 --> "같은 역 + 같은 날짜 + 같은 시간대 + 같은 승/하차" --> 1개만
    __table_args__ = (
    ...
    def __repr__(self):
        ...
        보통은 메모리 주소만 나오는데 __repr__d을 만들어주면 디버깅할 때 어떤 데이터인지 알아보기 쉬워진다.
        """
