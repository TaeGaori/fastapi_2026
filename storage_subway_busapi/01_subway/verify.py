...
def verify():
    ...
    with engine.connect() as conn:
        ...
        # --- 1. 전체 적재 건수 확인 ---
        # .scalar() --> 결과에서 첫번쨰 행의 첫번째 컬럼 값 하나만 뽑아온다
        #               (값이 하나만 나오는 쿼리에 사용)
        total = conn.execute(text("SELECT COUNT(*) FROM subway_raw")).scalar()

        # --- 2. 필수 컬럼별 NULL 개수 확인 ---
        # FILTER (WHERE 조건) --> 하나의 SELECT 안에서 조건 별로 COUNT를 따로 집계하는
        #                           PostgreSQL의 문법
        ...
        print(f'인원수 음수 건수 NULL 건수 : {negative_count}')
        print(f'시작시 범위 이탈 건수 NULL 건수 : {invalid_hour}')
        print(f'승하차 이상값 건수 NULL 건수 : {invalid_inout}')
        print(f'중복 키 건수 NULL 건수 : {duplicate_count}')
