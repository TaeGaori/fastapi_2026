...
# 동적 경롷 사용 --> /users/1, /users/2 ..처럼 사용자 아이디마다
...
def read_items_handler(max_price: int | None=None):
    # int일수도 있고, None일 수도 있으며, 기본밗은 None으로 설정
    return{'max_price': max_price}
# 경로 변수와 퀴리 파라미터는 둘 다 엔드포인트 함수의 매개변수로 사용된다.
# 경로 변수는 경로에서 값을 추출
# 퀴리 파라미터는 경로 뒤에 ?key=value 형태로 덧붙은 값을 추출