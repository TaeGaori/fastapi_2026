...
class Book(Base):
    ...
    # ISBN은 OCR 인식에 실패하며 ㄴ아직 모를 수 있으니 None으로 허용
    # String(13) --> 최대 13자까지 저장가능, 현재 ISBN기준
    # unique=True --> 중복 방지
    ...
    # 사진 파일 자체는 DB에 넣지 않고ㅡ 파일 시스템에 두고 경로만 기록(일반적인 방식)
    cover_path: Mapped[str | None] = mapped_column(String(500))
    # OCR검증이 성공했는지 실패했는지 상태를 나타내는 문자열
    # default='confirmed'  --> rlqhsrkqtdms 'confirmed', confiremed(확정)/needs_review(확인 필요)
    ...
    # uselist=False --> 이 책 하나당 독서상ㅌ애는 딱 하나. 단일 객체, 1:1 관계
