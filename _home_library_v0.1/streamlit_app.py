...
API = 'http://localhost:8000'   # 우리 FastAPI 서버 주소, Streamlit이 이 주소로 HTTP 요청을 보내서 데이터를 주고 받는다.
...
#     # request.post(...) --> FastAPI의 POST /books/scan 주소로 파일을 실어서 요청을 보낸다.
...
# ----- and image 없애고, 버튼클릭 안쪽에서 image 유무를 명시적으로 분기 -----
if st.button('등록'):
    if image is None:
        # 사진이 없는데 등록을 눌럿을 떄 --> 경고만 띄우고 끝
