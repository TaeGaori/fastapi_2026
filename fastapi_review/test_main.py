...
# 테스트용 세션 팩토라 --> database.py의 SessionLocal과 동이랗ㄴ 구조, DB만 다름
...
def setup_db():
    ...
    yield 위 : 테스트 전 준비 (테이블 생성) --> reviewdb_test에 테이블 생성
                Base를 상송한 모든 모델(User, Item)의 테이블이 생성된다.
...
def test_create_user(client):
    """유저 생성 -> 200 응답, 이메일 일치, 비밀번호 미노출, items 빈리스트 확인"""
...
def test_create_user_duplicate_email(client):
    """같은 이메일로 재가입 시도 -> 400에러 + 에러 메시지 확인"""
    response = client.post(
        '/users/',
        json={
...
def test_user_has_items(client):
    ...
    respnse = client.get('/users/1')

    assert respnse.status_code == 200

    data = respnse.json()
    # 위에서 아이템 1개를 생성 -> item가 비어있지 않아야 한다.
...
def test_read_items(client):
    """전체 아이템 목록 조회 -> 200응답, 리스트 타입, 1개 이상 확인"""
