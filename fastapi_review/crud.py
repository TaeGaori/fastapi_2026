...
import todo.models as models, schemas # fastapi_review/안에 있는 파일들 models.py, schemas.py

pwd_hasher = PasswordHash.recommended() # pwdlib이 권장하는 해싱 알고리즘(argon2) 자동 선택

# User CRUD
def get_user(db: Session, user_id: int):
    # db.query(모델) : 해당 테이블 전체를 다상으로 쿼리 시작
...
def get_users(db: Session, skip: int = 0, limit: int = 100):
    # .offset(skip): 앞에서 skip개 건너뜀 (페이지네이션 : 데이터를 한번에 다 보여주지 않고 페이지 단위로 나눠서 보여주는 것 )
    # (페에지네이션: 데이터를 한번에 다 보여주지 않고 페이지 단위로 나눠서 보여주는 것)
    # .limit(limit): 최대 limit개만 반환
    # .all(): 조건에 맞는 전체 결과를 리스트로 반환
    return db.query(models.User).offset(skip).limit(limit).all()

def create_user(db: Session, user: schemas.UserCreate):
    # 평문 비밀번호(user.password)를 해싱(원래 값을 알 수 없는 암호화된 무자열로 변환)하여 저장
    ...
    db.refresh(db_user) # Db가 자동 생성환 id 등의 값을 객체에 다시 채워넣는다.
...
def create_user_item(db: Session, item: schemas.ItemCreate, user_id: int):
    # iten.model_dump(): Pydantic객체 - 딕셔너리로 변환 {'title':..., 'description':...}
